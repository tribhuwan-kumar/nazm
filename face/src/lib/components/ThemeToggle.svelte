<script lang="ts">
  ...
  let { class: className = "", ...rest }: ThemeToggleProps = $props();
  let isPressed = $state(false);
  let transitionTimer: ReturnType<typeof setTimeout> | undefined;

  const syncThemeState = () => {
    isPressed = mode.current === "dark";
  };
  ...
  const toggle = async (event: MouseEvent) => {
    ...
    const isDarkBefore = mode.current === "dark";

    const transition = document.startViewTransition(async () => {
      toggleMode();
      await tick(); /* Ensure dom updates are flushed before capturing the new state */
    });
...
<style>
/* View Transition API Appearance Animation rules */
...
:root {
  ...
  --speed: 0.5s;
	--width: clamp(30px, 40vmin, 90px);
  --ar: 8 / 3;

	--ray: hsl(0 0% 100% / 0.5);
	--sun: #f1d9f1;
	--moon: #fcf6fc;;
  --crater: color-mix(in oklab, #DDA0DD 40%, transparent);
  --bg: oklch(0.542 0.034 322.5);
  --bear-speed: 10s;
...
.toggle {
  ...
  background: var(--bg);
  box-shadow:
    calc(var(--width) * 0) calc(var(--width) * 0.02) calc(var(--width) * 0.01) calc(var(--width) * -0.0025) var(--border),
    calc(var(--width) * 0) calc(var(--width) * -0.02) calc(var(--width) * 0.01) calc(var(--width) * -0.0025) var(--ring),
...
.toggle__indicator {
  ...
  place-items: center;
  padding: 3%;
  transition: transform var(--speed) var(--slide-ease);
  transform: translateX(calc(var(--dark, 0) * (100cqi - 100%)));
}
